...
birds_GMBA <- checklist %>%
  filter(group == "birds" & expert_validated == "yes") %>%
  distinct()

# Calculate range size
birds_GMBA <- birds_GMBA %>%
  mutate(GMBA_range = max_elevation - min_elevation)


# ------
# Import and process GBIF dataset
birds_GBIF <- open_dataset("GBIF/data/Aves_parquetclean")
# download DEM
...
pts_GBIF <- birds_GBIF[, .(decimalLongitude, decimalLatitude)]
# Add elevation
chunk_size <- 1e6
birds_GBIF[, elevation := NA_real_]  # data.table syntax + NA_real sets correct numerical type
for (i in seq(1, nrow(birds_GBIF), by = chunk_size)) {
  idx <- i:min(i + chunk_size - 1, nrow(birds_GBIF))
  birds_GBIF[idx, elevation := terra::extract(dem, pts_GBIF[idx, ])[, 2]]
  cat("Processed", max(idx), "/", nrow(birds_GBIF), "\n")
}

# Calculate elevation max and min + range size for each species in each mountain range
birds_GBIFrange <- birds_GBIF %>%
...
# ------
# Compare range size
birds_global <- semi_join(birds_GMBA, birds_GBIFrange, by = c("Mountain_range" = "Level_03", "sciname" = "species")) %>% # select only species found in Lotta's dataset
  left_join(birds_GBIFrange, birds_GMBA, by = c("Mountain_range" = "Level_03", "sciname" = "species"))

birds_global <- birds_global %>%
...
birds_GBIF <- birds_GBIF %>%
  semi_join(birds_global, by = c("species" = "sciname",
                                   "Level_03" = "Mountain_range"))

# Reference species from raw occurrences
reference_species <- birds_GBIF %>%
  group_by(species, Level_02, Level_03) %>%
  summarise(n = n(), .groups = "drop") %>%
  filter(n >= min_occ)

# True range from birds_global (GMBA values)
true_ranges <- birds_global %>%
